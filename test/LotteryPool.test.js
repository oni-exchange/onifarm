...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
  ...
  it('real case', async () => {
    ...
    assert.equal(
      (await this.oni.balanceOf(this.lottery.address)).toString(),
      '0'
    );
    ...
    assert.equal((await this.oni.balanceOf(carol)).toString(), '5');
  });
  ...
  it('setReceiver', async () => {
    ...
    assert.equal((await this.oni.balanceOf(carol)).toString(), '7');
    await this.lottery.setReceiver(alice, { from: dev });
    assert.equal((await this.lottery.pendingReward('1')).toString(), '7');
    await this.lottery.harvest(1, { from: dev });
    assert.equal((await this.oni.balanceOf(alice)).toString(), '15');
  });
