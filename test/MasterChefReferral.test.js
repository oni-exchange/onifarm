...
const MasterChefReferral = artifacts.require('MasterChefReferral');
const MockBEP20 = artifacts.require('libs/MockBEP20');

contract('MasterChefReferral', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.oni = await OniToken.new({ from: minter });
        this.oni.mint(alice, '1000', { from: minter });
        this.oni.mint(bob, '1000', { from: minter });
        this.oni.mint(carol, '1000', { from: minter });
        this.oni.mint(dev, '1000', { from: minter });
        this.oni.mint(minter, '1000', { from: minter });
...
//        this.syrup = await SyrupBar.new(this.oni.address, { from: minter });
        ...
        const feeAddress = carol;
        this.chef = await MasterChefReferral.new(this.oni.address, dev, feeAddress, 0, { from: minter });
...
//      await this.chef.add('100', this.lp3.address, 0, true, { from: minter });
      ...
      assert.equal((await this.oni.balanceOf(alice)).toString(), '263');

      await this.oni.approve(this.chef.address, '1000', { from: alice });
      await this.chef.enterStaking('20', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.oni.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.oni.balanceOf(alice)).toString(), '999');
      assert.equal((await this.oni.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.oni.balanceOf(bob)).toString(), '125');
    ...
    it('staking/unstaking', async () => {
      ...
      await this.oni.approve(this.chef.address, '250', { from: alice });
...
//      assert.equal((await this.syrup.balanceOf(alice)).toString(), '0');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.oni.approve(this.chef.address, '100', { from: alice });
      await this.oni.approve(this.chef.address, '100', { from: bob });
...
//      await this.chef.enterStaking('0', { from: bob });
      ...
      assert.equal((await this.oni.balanceOf(alice)).toString(), '700');
      assert.equal((await this.oni.balanceOf(bob)).toString(), '150');
...
//      await this.chef.leaveStaking('100', { from: bob });
    ...
    it('should allow dev and only dev to update dev', async () => {
        ...
        await expectRevert(this.chef.setDevAddress(bob, { from: bob }), 'dev: wut?');
        await this.chef.setDevAddress(bob, { from: dev });
        assert.equal((await this.chef.devAddr()).valueOf(), bob);
        await this.chef.setDevAddress(alice, { from: bob });
